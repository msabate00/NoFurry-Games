...
update_status ModuleInputName::Update()
{
	...
	if (App->input->keys[SDL_SCANCODE_A] == KEY_DOWN || pad.l_x < -0.2f || pad.left_down) {
	...
	if (App->input->keys[SDL_SCANCODE_D] == KEY_DOWN || pad.l_x > 0.2f || pad.right_down) {
		if (dardosPositionX == 215 && dardosPositionY == 35) {}
		else {
	...
	if (App->input->keys[SDL_SCANCODE_J] == KEY_DOWN || pad.a_down ) {
		if (dardosPositionX == 215 && dardosPositionY == 35) {
	...
	if (App->input->keys[SDL_SCANCODE_X] == KEY_DOWN || pad.b_down) {
		deleteNom();
	}
