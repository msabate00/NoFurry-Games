...
void Enemy::Update()
{
		...
		if (bang && facingLeft) 
		{
			App->particles->AddParticle(App->particles->enemyShotL, position.x, position.y - currentAnim->GetCurrentFrame().h + 76, Collider::Type::ENEMY_SHOT, 0);
		}

		else if (bang && !facingLeft)
		{
			App->particles->AddParticle(App->particles->enemyShotR, position.x + 20, position.y - currentAnim->GetCurrentFrame().h + 76, Collider::Type::ENEMY_SHOT, 0);
		}
